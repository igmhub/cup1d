...
from cup1d.models.igm import thermal_model
...
class TestSimulation(object):
    ...
    def __init__(self, basedir, sim_label, z_max=4.0, kp_Mpc=0.7):
    ...
    def _read_json_files(self, z_max, kp_Mpc):
        ...
        sim_config = read_gadget.read_gadget_paramfile(
            self.fulldir + "/sim_plus/paramfile.gadget"
        )
        ...
        drop = zs[
            : (len(zs) - len(self.zs))
        ]  ## Redshifts we don't want to keep
        ...
        # setup CAMB object
        self.sim_cosmo = camb_cosmo.get_cosmology_from_dictionary(
            sim_cosmo_dict
        )
        ...
        for aa in reversed(range(len(drop), len(zs))):
            ...
            assert len(k_Mpc_plus) == len(
                k_Mpc_minus
            ), "k bins are different in plus and minus sims"
            ...
            sigT_Mpc = 0.5 * (
                plus_data["sim_sigT_Mpc"] + minus_data["sim_sigT_Mpc"]
            )

            ## Find paired P_1D
            p1d_combined = (
                0.5
                * (p1d_plus * mf_plus**2 + p1d_minus * mf_minus**2)
                / pair_mf**2
            )
