...
for ii in range(nsam):
    if ("kF_Mpc" not in sim_test[ii]) | (
        np.isfinite(sim_test[ii]["kF_Mpc"]) == False
    ):
        continue

    p1d_Mpc_sim[ii] = sim_test[ii]["p1d_Mpc"][ind]
    p1d_Mpc_emu[ii] = emulator.emulate_p1d_Mpc(sim_test[ii], k_Mpc)

    norm = np.interp(
        k_Mpc,
        emulator.input_norm["k_Mpc"],
        emulator.norm_imF(sim_test[ii]["mF"]),
    )
...
# ax.set_xlim(0.08, 4)
ax.set_ylabel(
    r"$P_\mathrm{1D}^\mathrm{sim}/P_\mathrm{1D}^\mathrm{smooth}-1$",
    fontsize=ftsize,
)
