...
for ii in range(nax):
    ...
    for kk in range(2):
        _ = suite == kk
        ax[ii].scatter(x0[_], y0[_], marker=".", color="C"+str(kk))
...
for ii in range(nax):
    ...
    for kk in range(2):
        ...
        _ = suite == kk
        col = "C"+str(2*kk+1)
        ax[kk, ii].scatter(best_star[_, jj0], best_star[_, jj1], color=col, marker="o")
        col = "C"+str(2*kk)
        ax[kk, ii].scatter(fid_star[_, jj0], fid_star[_, jj1], color=col, marker="s")
    for kk in range(len(sim_labels)):
        if (ii != 0) and (suite[kk] == 1):
            continue
        ax[suite[kk], ii].annotate(sim_labels[kk], (fid_star[kk, jj0] + sep_x, fid_star[kk, jj1]), fontsize=8)
        col = "C"+str(2*suite[kk])
            
        ax[suite[kk], ii].plot([fid_star[kk, jj0], best_star[kk, jj0]], [fid_star[kk, jj1], best_star[kk, jj1]], col+"--", alpha=0.5)

    for kk in range(2):
        ax[kk, ii].axhline(true_star[0, jj1], color="black", linestyle=":", alpha=0.3)
