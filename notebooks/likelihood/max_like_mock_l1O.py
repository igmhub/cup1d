...
emulator = NNEmulator(
    ...
    model_path="NNmodels/emulator_" + sim_out + ".pt",
    train=False,
)
testing_data = archive.get_testing_data(sim_label="mpg_" + sim_out)

# %% [markdown]
# ### check precision

# %%
p1ds_true = np.zeros(shape=(11, 42))
p1ds = np.zeros(shape=(11, 42))
...
for m in range(11):
    ...
    fit_p1d = poly_p1d.PolyP1D(
        kMpc_test, p1d_true, kmin_Mpc=1e-3, kmax_Mpc=4, deg=5
    )
    ...
    plt.plot(
        kMpc_test,
        p1d / p1d_true,
        label="z=" + str(np.round(testing_data[m]["z"], 2)),
    )

plt.errorbar(
    kMpc_test,
    np.mean(p1ds / p1ds_true, axis=0),
    np.std(p1ds / p1ds_true, axis=0),
    color="k",
)
