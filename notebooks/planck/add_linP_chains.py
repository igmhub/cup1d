...
# %%
# Use the lightweight Planck chain stored with cup1d. The BAO version of this
# curved-cosmology chain is not included in the repository.
...
samples.addDerived(
    linP_DL2_star, "test_linP_DL2_star", label="Ly\\alpha \\, \\Delta_\\ast"
)
...
print(
    "n_star mean = {} +/- {}".format(
        param_means[Npar + 1], np.sqrt(param_vars[Npar + 1])
    )
)
...
print(
    "old DL2_star mean = {} +/- {}".format(param_means[Npar], np.sqrt(param_vars[Npar]))
)
print(
    "new DL2_star mean = {} +/- {}".format(
        new_param_means[Npar], np.sqrt(new_param_vars[Npar])
    )
)
