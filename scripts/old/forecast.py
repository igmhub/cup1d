...
parser.add_argument(
    "--emu_type", type=str, default="polyfit", help="k_bin or polyfit emulator"
)
parser.add_argument(
    "--emu_cov_factor",
    type=float,
...
parser.add_argument(
    "--kmax_Mpc", type=float, default=8, help="Maximum k to train emulator"
)
...
parser.add_argument(
    "--no_igm", action="store_true", help="Do not vary IGM parameters at all"
)
parser.add_argument(
    "--cosmo_fid_label",
    type=str,
...
parser.add_argument(
    "--burn_in", type=int, default=200, help="Number of burn in steps"
)
parser.add_argument(
    "--prior_Gauss_rms", type=float, default=0.5, help="Width of Gaussian prior"
)
parser.add_argument(
    "--data_label",
    type=str,
...
parser.add_argument(
    "--rootdir", type=str, default=None, help="Root directory containing chains"
)
parser.add_argument(
    "--extra_p1d_label",
    type=str,
...
emu = gp_emulator.GPEmulator(
    training_set="Pedersen21", emu_type=args.emu_type, kmax_Mpc=args.kmax_Mpc
)

# generate mock P1D measurement
data = mock_data.Mock_P1D(
    emulator=emu, data_label=args.data_label, zmax=args.z_max
)

# check if we want to include high-resolution data
if args.extra_p1d_label:
    extra_p1d_data = mock_data.Mock_P1D(
        emulator=emu, data_label=args.extra_p1d_label, zmax=args.z_max
    )
else:
    extra_p1d_data = None
...
sampler = emcee_sampler.EmceeSampler(
    like=like, subfolder=args.subfolder, rootdir=rootdir
)
